...
export const TaskbarItem = (props: TaskbarItemProps) => {
    return (
        <div className={`taskbar-item taskbar-${props.task.id} ${props.task.id === appService.activeAppId ? 'active': ''} ${props.task.noHover ? 'no-hover' : ''}`} onClick={() => appService.addActiveApp(props.task)} title={props.task.hoverText ? props.task.hoverText : props.task.displayName}>
